...
create table task (
	...
	type int(10) default 0 not null comment "任务类型，用二进制表示，从低位开始，第一位代表是否普通任务，第二位代表是否宝箱任务,第三位代表是否分享任务，第四位代表是否金榜任务",
...
create table exchange (
	...
	alipay_acount varchar(255) comment "支付宝账号,当present_type=2时，必须有值",
	bank_acount  bigint(20) comment "银行账号,当present_type=3时，必须有值",
...
create table apk(
	...
	address varchar(255) not null comment "下载地址",
	lastmod_time datetime not null comment "上次修改时间",
	lastmod_userid int(10) not null comment "上次修改人",
